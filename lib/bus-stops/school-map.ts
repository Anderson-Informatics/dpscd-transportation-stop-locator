...
const SCHOOL_LOCATIONS_PATH = '/data/School_Locations_2026-27.geojson'
...
// The geojson ships as a static public asset, which a function container cannot read
// from disk, so it is fetched over HTTP from the site serving the request.
export async function loadSchoolLocations(baseUrl: string) {
  const res = await fetch(new URL(SCHOOL_LOCATIONS_PATH, baseUrl))
  if (!res.ok) {
    throw new Error(`Could not load school locations (${res.status} from ${SCHOOL_LOCATIONS_PATH})`)
  }
